...
***请注意：对于推杆和夹爪关节，只有 position 和 effort 字段生效，其他字段需要保持为0。***

- 对于推杆关节：position 单位为`米`， 数值范围 0 - 0.0254m，0代表完全缩回；effort 代表最大电流，数值范围 0 - 2.5 A。

- 对于夹爪关节：position 单位为`百分比`，数值范围 0 - 1.0，0代表完全夹紧；effort 代表最大电流的百分比，数值范围 0 - 1，最大值于夹爪中配置。
