...
#   - conda 环境含 ROS2 Humble
#
# 产出:
#   reports/  — JSON + Markdown 测试报告
#   ci_logs/  — 各组件日志 (失败时上传)
# ============================================================
set -euo pipefail
...
# 激活 conda
cecho "\n${B}[ci] 1/6 环境准备${N}"
for _conda_base in "$HOME/Anaconda" "$HOME/anaconda3" "$HOME/miniconda3" "/opt/conda"; do
    if [ -f "$_conda_base/etc/profile.d/conda.sh" ]; then
        source "$_conda_base/etc/profile.d/conda.sh"
        conda activate F1 2>/dev/null && break
    fi
done
...
" 2>/dev/null || echo "1")
    ...
    LATEST_JSON="$(ls -t "$REPORT_DIR"/*/result.json 2>/dev/null | head -1)"
